...
namespace SilentKassadin
{
    public static class Program
    {
        ...
        private static void Game_OnGameLoad(EventArgs args)
        {
            ...
            ShowNotification("SilentKassadin ready to fire.", Color.Crimson, 4000);
            ShowNotification("ManaManager by iJabba", Color.Crimson, 4000);
        }
        ...
        private static void OnCombo(Obj_AI_Hero target)
        {
            if (_menu.Item("useRiftWalk").GetValue<bool>() && _riftWalk.IsReady() &&
                _player.Distance(target.Position) <= _riftWalk.Range && target.IsValidTarget(_riftWalk.Range))
            {
                _riftWalk.CastIfHitchanceEquals(target, CustomHitChance);
            }

            if (_menu.Item("useNetherBlade").GetValue<bool>() && _netherBlade.IsReady() &&
                _player.Distance(target.Position)<= _netherBlade.Range + 25)
            {
                _netherBlade.Cast();
            }
            ...
            if (_menu.Item("useForcePulse").GetValue<bool>() && _forcePulse.IsReady() &&
                _player.Distance(target.Position) <= _forcePulse.Range && target.IsValidTarget(_forcePulse.Range))
            {
        ...
        {
            _menu = new Menu("Silent" + ChampionName, "silent" + ChampionName, true);
